...
if __name__ == '__main__':
    ...
    ba.init_exchange(user_main_1, user_main_2)
    logger.info("交易所初始化完毕,开始获取用户token...")
    ba.get_token(user_main_1, user_main_2)
    ba.获取交易对规则2(user_main_1, user_main_2)
    user_main_1.交易对价格精度 = 4
    user_main_2.交易对价格精度 = 4
    logger.info("获取用户token完毕,开始启动公有化线程获取用户交易对信息...")
    pub = PublicGridWebSocket(user_main_1, user_main_2)
    pub.run()
    logger.info("公有化线程启动完毕,开始启动私有化线程获取用户交易信息...")
    pri = PrivateGridWebSocket(user_main_1, user_main_2)
    pri.run()
