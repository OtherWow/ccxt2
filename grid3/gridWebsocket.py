...
class PublicGridWebSocket:
    def __init__(self, user_main_1: Account, user_hedge_1: Account, user_main_2: Account, user_hedge_2: Account,
        ...
        self.user_对冲 = user_对冲
        ba.撤销所有订单(self.user_对冲)
        ba.查询账户持仓情况(self.user_对冲)
        ba.市价平仓(self.user_对冲)
        ba.查询账户持仓情况(self.user_对冲)
    ...
    def on_message(self, ws, message):
        ...
        self.user_hedge_1.now_price = float(data['p'])
        self.user_hedge_2.now_price = float(data['p'])
        self.user_对冲.now_price = float(data['p'])
        if self.user_对冲.now_price >= self.user_对冲.中间价格:  # 当前价格大于中间价格时做多
            self.user_对冲.position_side = "LONG"
            if self.user_对冲.now_price >= self.user_对冲.做多触发价格 and self.user_对冲.position_amt == 0:

                # 当前价格高于做多触发价格 且对冲账号仓位为0时
                ba.查询账户持仓情况(self.user_对冲)
                if self.user_对冲.position_amt == 0:
                    logger.info("=============================达到触发条件，开始下做多对冲单=============================")
                    ba.市价单(self.user_对冲, "BUY", self.user_对冲.对冲单数量)
                    ba.限价止盈单(self.user_对冲, (self.user_对冲.做多止盈价格 - self.user_对冲.now_price) / 2 + self.user_对冲.now_price,
                             self.user_对冲.做多止盈价格)
                    ba.限价止损单(self.user_对冲, self.user_对冲.做多限价止损触发价格, self.user_对冲.做多限价止损价格)
                    ba.市价止损单(self.user_对冲, self.user_对冲.做多市价止损价格)
                    logger.info("=============================做多对冲单结束=============================")
        else:
            self.user_对冲.position_side = "SHORT"
            if self.user_对冲.now_price <= self.user_对冲.做空触发价格 and self.user_对冲.position_amt == 0:
                # 当前价格高于做多触发价格 且对冲账号仓位为0时
                ba.查询账户持仓情况(self.user_对冲)
                if self.user_对冲.position_amt == 0:
                    logger.info("=============================达到触发条件，开始下做多对冲单=============================")
                    ba.市价单(self.user_对冲, "SELL", self.user_对冲.对冲单数量)
                    ba.限价止盈单(self.user_对冲, self.user_对冲.now_price - (self.user_对冲.now_price - self.user_对冲.做空止盈价格) / 2,
                             self.user_对冲.做空止盈价格)
                    ba.限价止损单(self.user_对冲, self.user_对冲.做空限价止损触发价格, self.user_对冲.做空限价止损价格)
                    ba.市价止损单(self.user_对冲, self.user_对冲.做空市价止损价格)
                    logger.info("=============================做多对冲单结束=============================")
...
def 触发限价单则新增任务队列(data, user: Account, 配对user: Account):
    ...
    if data['e'] == 'ORDER_TRADE_UPDATE' and data['o']['o'] == 'LIMIT' and data['o']['x'] == 'TRADE' and data['o'][
        'X'] == 'FILLED' and (data['o']['s'] == user.symbol):  # 限价单成交
        if (user.position_side == "LONG" and data['o']['S'] == 'SELL') or (
                user.position_side == "SHORT" and data['o']['S'] == 'BUY'):
            配对user.已配对次数 += 1

        logger.info(
            f"{user.name}【{user.symbol}】：限价单{data['o']['i']}成交，成交价格：{data['o']['p']}，成交数量：{data['o']['z']}，成交方向：{data['o']['S']}，账号手续费：【{user.手续费}】，账号盈亏：【{user.盈亏}】，{user.name}【{user.symbol}】已配对【{配对user.已配对次数}】次")
...
class PrivateGridWebSocket:
    ...
    def run(self):
        ...
        threading.Thread(target=self.private_ws_对冲).start()
    ...
    def private_ws_对冲(self):
        websocket.enableTrace(True)
        self.ws_对冲 = websocket.WebSocketApp("wss://fstream.binance.com/ws/" + self.user_对冲.listen_key,
                                            on_message=self.user_对冲_message)
        self.ws_对冲.run_forever(sslopt={"check_hostname": False})
    ...
    def user_对冲_message(self, ws, message):
        data = json.loads(message)
        if data['e'] == 'ACCOUNT_UPDATE':
            for temp in data['a']['P']:
                if temp['s'] == self.user_对冲.symbol:
                    pa = abs(float(temp['pa']))
                    self.user_对冲.position_amt = abs(float(temp['pa']))
                    if pa == 0:
                        ba.撤销所有订单(self.user_对冲)
                        ba.查询账户持仓情况(self.user_对冲)
                        ba.市价平仓(self.user_对冲)
